...
public class Dwc extends Script implements PaintListener, MouseListener, MouseMotionListener, MessageListener {
	...
	public static final int notedOakId = 1522;
	...
	final int[] invOkIds = { regularLogId, oakLogId, willowLogId, yewLogId, 995, bronzeHatchetId, ironHatchetId,
	        steelHatchetId, blackHatchetId, mithrilHatchetId, adamantHatchetId, runeHatchetId, notedOakId, tinderboxId,
	        14664, 13439 }; // Random event lamp 14664, Starter lamp 13439
	final int[] dontDepositIds = { bronzeHatchetId, ironHatchetId, steelHatchetId, blackHatchetId, mithrilHatchetId,
	        adamantHatchetId, runeHatchetId, tinderboxId };
	...
	public int loop() {
		...
		switch (getState()) {
			...
			case WILLOWRIMM:
				...
				if (calc.distanceTo(draynorBankTile) < 40 && inventory.getCount(invOkIds) > 10) {
					checkBank = true;
					return 0;
				}
	...
	private int dropLogs() {
		...
		for (int i : chosenPath) {
			...
			inner: for (int id : logIds) {
				if (log.getID() == id) {
					if (log != null && !log.interact("Drop"))
	...
	private int chopTree(final int farDist, final int closeDist, final RSTile t, final String treeName,
		...
		if (powerchop && !trainFM) {
			...
			if (!dropTimer.isRunning() && logCount < logs
				...
				mouse.click(false);
				sleep(random(10, 40));
				mouse.hop(mouse.getLocation().x, mouse.getLocation().y + 37);
				sleep(random(10, 40));
				mouse.click(true);
				logCount = logs - 1;
				clickTree = true;
	...
	private boolean canBurn(final RSItem l) {
		...
		if (l.getID() == willowLogId)
			return fmLvl() >= 30 ? true : false;
		if (l.getID() == oakLogId)
			return fmLvl() >= 15 ? true : false;
		if (l.getID() == regularLogId)
			return fmLvl() >= 1 ? true : false;
		return false;
	}
	/**
	 * @return True, if the player is not moving and has no animation.
	 */
	private boolean isIdle() {
		return !isAnimated() && !getMyPlayer().isMoving();
	}
	/**
	 * @return True, if the local player is animated.
	 */
	...
	/**
	 * @param t
	 * @return True if there is a no fire on the tile, otherwise false.
	 */
	...
	/**
	 * @return The highest tier hatchet in the players inventory or bank for the current level.
	 */
	...
	final Color colorShadow = new Color(20, 20, 20, 100);
	final BasicStroke stroke1 = new BasicStroke(1);
	final Font arialS = new Font(Font.DIALOG, 0, 9);
	final Font arialL = new Font(Font.DIALOG, 0, 12);
	final Font title = new Font(Font.SANS_SERIF, Font.PLAIN, 16);
	String dots = "";
	final BufferedImage imgFlameSelected = getImage("flameSelected.png", "http://i.imgur.com/6rlRE.png");
	final BufferedImage imgFlame = getImage("flame.png", "http://i.imgur.com/5RwSx.png");
	final BufferedImage imgLocationSelected = getImage("globeSelected.png", "http://i.imgur.com/uQBlH.png");
	final BufferedImage imgLocation = getImage("globe.png", "http://i.imgur.com/EP3ZX.png");
	final BufferedImage imgInfoSelected = getImage("infoSelected.png", "http://i.imgur.com/coTV7.png");
	final BufferedImage imgInfo = getImage("info.png", "http://i.imgur.com/RfOUW.png");
	...
	final BufferedImage imgSettingsSelected = getImage("settingsSelected.png", "http://i.imgur.com/shwXk.png");
	final BufferedImage imgSettings = getImage("settings.png", "http://i.imgur.com/wgaN0.png");
	...
	public void onRepaint(final Graphics g1) {
		...
		if (isLoggedIn()) {
			...
			if (globeSelected) {
				g.drawImage(imgLocationSelected, 490, 5, null); // Globe
				if (!locked)
					g.drawImage(imgLockOpen, 462, 5, null);
				else
					g.drawImage(imgLockClosed, 462, 5, null);
			} else
				g.drawImage(imgLocation, 490, 5, null);
	...
	class Paint {
		...
		public void drawPaint(Graphics g1) {
			...
			if (showPaint) {
				...
				g.drawString("Time: " + a.format(hours) + ":" + a.format(minutes) + ":" + a.format(seconds), x + 5,
				    y + 48);
				...
				g.setFont(arialL); // Hide button
				g.drawRect(x + w - 34, y + h - 19, 34, 19);
				if (showSelected) {
					g.setColor(colorShadow);
					g.drawString("Hide", x + w - 30, y + h - 5);
				}
				g.setColor(Color.BLACK);
				g.drawString("Hide", x + w - 29, y + h - 4);
			} else {
				g.setColor(colorGreenL); // Show button
				...
				if (showSelected) {
					g.setColor(colorShadow);
					g.drawString("Show", x + w - 33, y + h - 5);
				}
				g.setColor(Color.BLACK);
					...
					if (!changePrice && !changeQuantity) {
						while (isOpen()) {
							if (interfaces.getComponent(GE_INTERFACE, 187).getText() != null)
								if (interfaces.getComponent(GE_INTERFACE, 187).doClick()) {
									close();
	...
	private BufferedImage getImage(final String fileName, final String imageURL) {
		try {
			final File dir = new File(Configuration.Paths.getScriptCacheDirectory() + File.separator
			        + "DynamicWoodcutter");
			if (!dir.exists())
				dir.mkdir();
			final File dir2 = new File(dir + File.separator + "Images");
			if (!dir2.exists())
				dir2.mkdir();
			final File f = new File(dir2 + File.separator + fileName);
			if (!f.exists()) {
